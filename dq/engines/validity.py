# Validity engine — rule-based format / range / cross-field validation (pure SQL).
...
log = logging.getLogger("dq.engines.validity")

# Dead: `RULE_META = VAL_RULE_META` / `TABLE_RULES = VAL_TABLE_RULES` aliases
# used to be exported here "for resolution_pipeline._build_dispatch" — that
# function doesn't exist anywhere in the codebase (confirmed via grep), and no
# other module imports validity_check.RULE_META/TABLE_RULES either. Dropped.
...
def _val_rule_sql(rule_id: str, existing: set) -> tuple[str, str] | None:
    ...
    if rule_id == "VAL-030":            # RWF disbursement amount >= 1000
        if not has("currency", "current_disbursed_amt"):
            return None
        return (
            'SUM(CASE WHEN "currency" IS NOT NULL AND UPPER(TRIM("currency"::TEXT)) = \'RWF\' '
            'AND "current_disbursed_amt" IS NOT NULL THEN 1 ELSE 0 END)',
            'SUM(CASE WHEN "currency" IS NOT NULL AND UPPER(TRIM("currency"::TEXT)) = \'RWF\' '
            'AND "current_disbursed_amt" IS NOT NULL '
            'AND "current_disbursed_amt"::NUMERIC >= 1000 THEN 1 ELSE 0 END)',
        )

    if rule_id == "VAL-031":            # foreign-currency disbursement amount >= 1
        if not has("currency", "current_disbursed_amt"):
            return None
        return (
            'SUM(CASE WHEN "currency" IS NOT NULL AND UPPER(TRIM("currency"::TEXT)) <> \'RWF\' '
            ...
            'SUM(CASE WHEN "currency" IS NOT NULL AND UPPER(TRIM("currency"::TEXT)) <> \'RWF\' '
            'AND "current_disbursed_amt" IS NOT NULL '
            'AND "current_disbursed_amt"::NUMERIC >= 1 THEN 1 ELSE 0 END)',
        )

    if rule_id == "VAL-032":            # disbursed loan amount >= 1000
        if not has("disbursed_amount"):
            return None
        return (
            'SUM(CASE WHEN "disbursed_amount" IS NOT NULL THEN 1 ELSE 0 END)',
            'SUM(CASE WHEN "disbursed_amount" IS NOT NULL AND "disbursed_amount"::NUMERIC >= 1000 THEN 1 ELSE 0 END)',
        )

    if rule_id == "VAL-033":            # distributed (principal) contract amount >= 1000
        if not has("principal_amount_lcy"):
            return None
        return (
            'SUM(CASE WHEN "principal_amount_lcy" IS NOT NULL THEN 1 ELSE 0 END)',
            'SUM(CASE WHEN "principal_amount_lcy" IS NOT NULL AND "principal_amount_lcy"::NUMERIC >= 1000 THEN 1 ELSE 0 END)',
        )

    if rule_id == "VAL-034":            # application date must not be after business date
        if not has("application_date", "business_date"):
            return None
        return (
            'SUM(CASE WHEN "application_date" IS NOT NULL AND "business_date" IS NOT NULL THEN 1 ELSE 0 END)',
            'SUM(CASE WHEN "application_date" IS NOT NULL AND "business_date" IS NOT NULL '
