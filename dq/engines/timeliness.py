...
def _tim_rule_sql(rule_id: str, existing: set) -> "tuple[str, str] | None":
    ...
    column(s) are unavailable (the rule is skipped for that table)."""
...
def main():
    ...
    rep = evaluate_from_sql(engine, args.schema, vlb, 0, {},
                            args.output, row_limit=args.limit)
    log.info("DONE → overall timeliness: %.2f%%",
             rep["executive_summary"]["overall_timeliness_score"])
    print(json.dumps({"overall": rep.get("executive_summary", {}).get("overall_timeliness_score"),
