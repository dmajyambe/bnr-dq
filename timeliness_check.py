...
#     FRESHNESS_WINDOW_DAYS, APPLICATION_SLA_DAYS,
...
#     # ── TIM-015: account_closing_date > account_open_date ─────────────────────
#     if rule_id == "TIM-015":
#         return _ordered_pair(df, "account_open_date", "account_closing_date", strict=True)

#     # ── TIM-016: account_status_date >= account_open_date ─────────────────────
#     if rule_id == "TIM-016":
#         return _ordered_pair(df, "account_open_date", "account_status_date", strict=False)

#     # ── TIM-017: business_date >= application_date ────────────────────────────
#     if rule_id == "TIM-017":
#         return _ordered_pair(df, "application_date", "business_date", strict=False)

#     # ── TIM-018: under-review apps must be resolved within SLA_DAYS ──────────
#     if rule_id == "TIM-018":
#         needed = ["application_status", "business_date", "application_date"]
#         if not all(c in df.columns for c in needed):
#             return None
#         status = df["application_status"].astype(str).str.strip().str.upper()
#         scope  = df[status == "U"].copy()
#         if scope.empty:
#             return None
#         bd = _to_dt(scope["business_date"])
#         ad = _to_dt(scope["application_date"])
#         pair = pd.DataFrame({"bd": bd, "ad": ad}).dropna()
#         if pair.empty:
#             return None
#         days = (pair["bd"] - pair["ad"]).dt.days.clip(lower=0)
#         valid = int((days <= APPLICATION_SLA_DAYS).sum())
#         return valid, len(pair) - valid, len(pair)

#     # ── TIM-019: approved/rejected apps must be decided within SLA_DAYS ───────
#     if rule_id == "TIM-019":
#         needed = ["application_status", "business_date", "application_date"]
#         if not all(c in df.columns for c in needed):
#             return None
#         status = df["application_status"].astype(str).str.strip().str.upper()
#         scope  = df[status.isin({"A", "R"})].copy()
#         if scope.empty:
#             return None
#         bd = _to_dt(scope["business_date"])
#         ad = _to_dt(scope["application_date"])
...
#     if rule_id == "TIM-015":
#         if not all(c in df.columns for c in ("account_open_date", "account_closing_date")): return false
#         a = _to_dt(df["account_open_date"])
#         b = _to_dt(df["account_closing_date"])
#         return a.notna() & b.notna() & (b <= a)

#     if rule_id == "TIM-016":
#         if not all(c in df.columns for c in ("account_open_date", "account_status_date")): return false
#         a = _to_dt(df["account_open_date"])
#         b = _to_dt(df["account_status_date"])
#         return a.notna() & b.notna() & (b < a)

#     if rule_id == "TIM-017":
#         if not all(c in df.columns for c in ("application_date", "business_date")): return false
#         ad = _to_dt(df["application_date"])
#         bd = _to_dt(df["business_date"])
#         return ad.notna() & bd.notna() & (bd < ad)

#     if rule_id == "TIM-018":
#         if not all(c in df.columns for c in ("application_status", "business_date", "application_date")): return false
#         status = df["application_status"].astype(str).str.strip().str.upper()
#         bd = _to_dt(df["business_date"])
#         ad = _to_dt(df["application_date"])
#         days = (bd - ad).dt.days.clip(lower=0)
#         return (status == "U") & bd.notna() & ad.notna() & (days > APPLICATION_SLA_DAYS)
...
#         ad = _to_dt(df["application_date"])
#         days = (bd - ad).dt.days.clip(lower=0)
#         return status.isin({"A", "R"}) & bd.notna() & ad.notna() & (days > APPLICATION_SLA_DAYS)
